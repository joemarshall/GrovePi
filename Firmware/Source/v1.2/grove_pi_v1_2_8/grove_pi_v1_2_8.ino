...
void loop()
{
  ...
  if (index == 4 && flag == 0 )
  {
    ...
    else if (cmd[0] == pulse_read_cmd )
    {
      int pin = cmd[1];
      int bpm = ps.getHeartRate(pin);
      boolean hasBeat = ps.hasBeatHappened(pin);
      b[0] = 0;
      b[1] = hasBeat;
      b[2] = bpm % 256;
      b[3] = (bpm >> 8) & 256;
...
void sendData()
{
  ...
  if (cmd[0] == 3 || cmd[0] == 7 || cmd[0] == 56)
    Wire.write(b, 3);
  if (cmd[0] == 8 || cmd[0] == 20 || cmd[0] == pulse_read_cmd )
    Wire.write(b, 4);
  if (cmd[0] == 30)
    Wire.write(b, 9);
  if (cmd[0] == 40)
